app.controller("myCtrl", function ($scope, $http, $filter) {
    ...
    $scope.GetWaitingListForDeputySecretary = function () {
    ...
    $scope.showTrustedhtml = function (row) {
        alert('Zoom');
    };
    ...
    $scope.rowDblClickCompCons = function (row) {
        ...
        //$scope.ApproveDetail = row.entity.MemberResolutionDetail;
        //$scope.AcceptanceComment = row.entity.AcceptanceComment;
        //$scope.MemberResolutionDetail = row.entity.html;//MemberResolutionDetail;
    ...
    $scope.GetSearchForEditDeputySecretary = function () {
        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetSearchForEditDeputySecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetPostedHistoryListByDeputySecretary = function () {
        $scope.gridDepartmentOptions1.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetPostedHistoryListByDeputySecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    var columnDepartmentList = [
        ...
        { name: 'RDNo', displayName: "আর ডি নং", width: 100 },
        { name: 'DeputySecApproveStatus', displayName: "Status", width: 120 },
        { name: 'AcceptanceComment', displayName: "গ্রহনযোগ্যতা", visible: false },
        { name: 'DeputySecApproveDetail', displayName: "Status", visible: false },
        { name: 'DeputySecApproveDate', displayName: "Status", visible: false },
    ...
    $scope.rowDblClickCompApproval = function (row) {
        ...
        $scope.MemberResolutionFIleURL = row.entity.MemberResolutionFIleURL;
        $scope.RDNo = row.entity.RDNo;
        $scope.frmDeputySecretaryApproval.ParliamentSession = row.entity.ParlSessID;

        $scope.ApproveDetail = row.entity.html;
        $scope.ApproveDate = $filter('FullDateTime')(row.entity.DeputySecApproveDate);
        $scope.frmDeputySecretaryApproval.AppStatus = row.entity.AprID;
    ...
    $scope.DirectSave = function (row) {
        ...
        $scope.SaveDb.DeputySecApproveDetail = (row.entity.html == '' || row.entity.html == null || row.entity.html == "null") ? row.entity.MemberResolutionDetail : row.entity.html;;
    ...
    $scope.MultipleSaveData = function () {
        ...
        for (var i = resolutionList.length - 1; i >= 0; i--) {
            ...
            if (resolutionList[i].Selected === true) {
                ...
                $scope.SaveDb.DeputySecApproveDetail = (resolutionList[i].html == '' || resolutionList[i].html == null || resolutionList[i].html == "null") ? resolutionList[i].MemberResolutionDetail : resolutionList[i].html;
                $scope.SaveDb.DeputySecApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
                $scope.SaveDb.DeputySecApproveStatus = "1";
                $scope.SaveDb.SendTo = $scope.frmDeputySecretaryApproval.SignTo;
    ...
    $scope.SaveData = function () {
        debugger;
        if ($scope.frmDeputySecretaryApproval.AppStatus === '' || $scope.frmDeputySecretaryApproval.AppStatus === undefined) {
            toastr.warning("সিদ্ধান্ত-প্রস্তাবের মতামত প্রদান করুন");
            return false;
        }
        if ($scope.frmDeputySecretaryApproval.SignTo === '' || $scope.frmDeputySecretaryApproval.SignTo === undefined) {
            toastr.warning("প্রাপোক নির্বাচন করুন");
            return false;
        }
        ...
        $scope.SaveDb.ParlSessID = $scope.frmDeputySecretaryApproval.ParliamentSession;

        $scope.SaveDb.DeputySecApproveDetail = $scope.ApproveDetail;
        $scope.SaveDb.DeputySecApproveDate = $scope.ApproveDate;
    ...
    $scope.Reset = function () {
        ...
        $scope.ApproveDetail = "";
        $scope.ApproveDate = "";
        $scope.frmDeputySecretaryApproval.ParliamentSession = "";
