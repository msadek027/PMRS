app.controller("myCtrl", function ($scope, $http, $filter) {
    ...
    $scope.GetWaitingListForAssistantSecretary = function () {
    ...
    $scope.GetSearchForEditAssistantSecretary = function () {

        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetSearchForEditAssistantSecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetPostedHistoryListByAssistantSecretary = function () {

        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetPostedHistoryListByAssistantSecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetDraft = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ResolutionApproval/GetWaitingListForAssistantSecretary",
            data: { session: $scope.frmAssistantSecretaryApproval.ParliamentSession, DataMode: "Draft" }
        }).then(function (response) {
            if (response.data.length > 0) {
                $('#DepartmentModal').modal('toggle');
                $scope.gridDepartmentOptions.data = response.data;
    ...
    var columnDepartmentList = [
        ...
        { name: 'AcceptanceComment', displayName: "গ্রহনযোগ্যতা", visible: false },
        { name: 'RDNo', displayName: "আর ডি নং", width: 100 },
        { name: 'AssitantSccApproveStatus', displayName: "Status", width: 120 },
        { name: 'AssitantSccDetail', displayName: "Status", visible: false },
        { name: 'AdministrativeOfcSignature', displayName: "প্রশাসনিক কর্মকর্তা", width: 120, cellTemplate: '<img src="{{row.entity.AdministrativeOfcSignature}}" alt="Not Signed" width="200">' },
    ...
    $scope.rowDblClickCompApproval = function (row) {
        ...
        $scope.AcceptanceComment = row.entity.AcceptanceComment;
        $scope.RDNo = row.entity.RDNo;
        $scope.frmAssistantSecretaryApproval.ParliamentSession = row.entity.ParlSessID;

        $scope.ApproveDetail = row.entity.html;

        $scope.ApproveDate = $filter('FullDateTime')(row.entity.AssitantSccDetail);
        $scope.frmAssistantSecretaryApproval.AppStatus = row.entity.AprID;
        $scope.GetResolutionLog(row.entity.MemberResolutionID);
    ...
    $scope.DirectSave = function (row) {
        ...
        $scope.SaveDb.AssitantSccDetail = (row.entity.html == '' || row.entity.html == null || row.entity.html == "null") ? row.entity.MemberResolutionDetail : row.entity.html;;
    ...
    $scope.MultipleSaveData = function () {
        ...
        for (var i = resolutionList.length - 1; i >= 0; i--) {
            ...
            if (resolutionList[i].Selected === true) {
                ...
                $scope.SaveDb.AssitantSccDetail = (resolutionList[i].html == '' || resolutionList[i].html == null || resolutionList[i].html == "null") ? resolutionList[i].MemberResolutionDetail : resolutionList[i].html;
                $scope.SaveDb.AssitantSccApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
                $scope.SaveDb.AssitantSccApproveStatus = "1";
                $scope.SaveDb.SendTo = $scope.frmAssistantSecretaryApproval.SignTo;
                if ($scope.uiID === '' || typeof $scope.uiID === 'undefined' && $scope.SaveDb.SendTo != '' && $scope.SaveDb.SendTo != 'undefined' && $scope.SaveDb.SendTo != undefined) {
    ...
    $scope.SaveData = function () {
        ...
        if ($scope.frmAssistantSecretaryApproval.AppStatus === '' || $scope.frmAssistantSecretaryApproval.AppStatus === undefined) {
            toastr.warning("সিদ্ধান্ত-প্রস্তাবের মতামত প্রদান করুন");
            return false;
        }
        if ($scope.frmAssistantSecretaryApproval.SignTo === '' || $scope.frmAssistantSecretaryApproval.SignTo === undefined) {
            toastr.warning("প্রাপোক নির্বাচন করুন");
            return false;
        }
        $scope.SaveDb = {};
        ...
        $scope.SaveDb.ParlSessID = $scope.frmAssistantSecretaryApproval.ParliamentSession;

        $scope.SaveDb.AssitantSccDetail = $scope.ApproveDetail;
        $scope.SaveDb.AssitantSccApproveDate = $scope.ApproveDate;
    ...
    $scope.SaveDataDraft = function () {
        ...
        $http({
            method: "post",
            url: MyApp.rootPath + "ResolutionApproval/DraftAssistantSecretary",
            datatype: "json",
            data: JSON.stringify($scope.SaveDb)
        }).then(function (response) {
            if (response.data.Status === "Yes") {
    ...
    $scope.Reset = function () {
        ...
        $scope.RDNo = "";
        $scope.frmAssistantSecretaryApproval.ParliamentSession = "";
