app.controller("myCtrl", function ($scope, $http, $filter) {
    ...
    $scope.GetWaitingListForSecretary = function () {
        $scope.gridResolutionOptions.data = [];
        $http({
            method: "POST",
            url: MyApp.rootPath + "ResolutionApproval/GetWaitingListForSecretary",
            data: { session: $scope.frmSecretaryApproval.ParliamentSession,DataMode: $scope.DataMode }
        }).then(function (response) {
    ...
    $scope.loadDataMode = function () {
        $scope.GetWaitingListForSecretary();
    }
    var columnResolutionList = [
        ...
        { name: 'BanglaName', displayName: "সদস্যের নাম", width: 120 },
        { name: 'RDNo', displayName: "আর ডি নং", width: 80 },
    ...
    $scope.GetSearchForEditSecretary = function () {
        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetSearchForEditSecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetPostedHistoryListBySecretary = function () {
        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetPostedHistoryListBySecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetDraft = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ResolutionApproval/GetWaitingListForSecretary",
            data: { session: $scope.frmSecretaryApproval.ParliamentSession, DataMode: "Draft" }
        }).then(function (response) {
            if (response.data.length > 0) {
                $('#DepartmentModal').modal('toggle');
                $scope.gridDepartmentOptions.data = response.data;
    ...
    var columnDepartmentList = [
        ...
        { name: 'SecApproveStatus', displayName: "Status", width: 120},
        { name: 'SecApproveDetail', displayName: "Status", visible: false },
        { name: 'SecApproveDate', displayName: "Status", visible: false },
        { name: 'AdministrativeOfcSignature', displayName: "প্রশাসনিক কর্মকর্তা", width: 120, cellTemplate: '<img src="{{row.entity.AdministrativeOfcSignature}}" alt="Not Signed" width="200">' },
        { name: 'AssitantSccSignature', displayName: "সহকারী সচিব", width: 120, cellTemplate: '<img src="{{row.entity.AssitantSccSignature}}" alt="Not Signed" width="200">' },
        { name: 'SrAssitantSccSignature', displayName: "সিনিয়র সহকারী সচিব", width: 120, cellTemplate: '<img src="{{row.entity.SrAssitantSccSignature}}" alt="Not Signed" width="200">' },
        { name: 'DeputySecSignature', displayName: "যুগ্ম সচিব", width: 120, cellTemplate: '<img src="{{row.entity.DeputySecSignature}}" alt="Not Signed" width="150">' },
    ...
    $scope.rowDblClickCompApproval = function (row) {
        ...
        $scope.AcceptanceComment = row.entity.AcceptanceComment;
        $scope.RDNo = row.entity.RDNo;
        $scope.frmSecretaryApproval.ParliamentSession = row.entity.ParlSessID;

        $scope.ApproveDetail = row.entity.html;
        $scope.ApproveDate = $filter('FullDateTime')(row.entity.SecApproveDate);
        $scope.frmSecretaryApproval.AppStatus = row.entity.AprID;
        $scope.GetResolutionLog(row.entity.MemberResolutionID);
    ...
    $scope.DirectSave = function (row) {
        ...
        $scope.SaveDb.SecApproveDetail = (row.entity.html == '' || row.entity.html == null || row.entity.html == "null") ? row.entity.MemberResolutionDetail : row.entity.html; row.entity.MemberResolutionDetail;

       $scope.SaveDb.SecApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
        $scope.SaveDb.SecApproveStatus = "1";
        $scope.SaveDb.SendTo = $scope.frmSecretaryApproval.SignTo;
        ...
        if ($scope.uiID === '' || typeof $scope.uiID === 'undefined' && $scope.SaveDb.SendTo != '' && $scope.SaveDb.SendTo != 'undefined' && $scope.SaveDb.SendTo != undefined) {
            $http({
                method: "post",
                url: MyApp.rootPath + "ResolutionApproval/UpdateSecApproval",
                datatype: "json",
    ...
    $scope.MultipleSaveData = function () {
        ...
        for (var i = resolutionList.length - 1; i >= 0; i--) {
            ...
            if (resolutionList[i].Selected === true) {
                ...
                $scope.SaveDb.SecApproveDetail = (resolutionList[i].html == '' || resolutionList[i].html == null || resolutionList[i].html == "null") ? resolutionList[i].MemberResolutionDetail : resolutionList[i].html;
                $scope.SaveDb.SecApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
                $scope.SaveDb.SecApproveStatus = "1";
                $scope.SaveDb.SendTo = $scope.frmSecretaryApproval.SignTo;
                $scope.SaveDb.DataMode = $scope.DataMode;
    ...
    $scope.SaveData = function () {
        debugger;
        if ($scope.frmSecretaryApproval.AppStatus === '' || $scope.frmSecretaryApproval.AppStatus === undefined) {
            toastr.warning("সিদ্ধান্ত-প্রস্তাবের মতামত প্রদান করুন");
            return false;
        }
        if ($scope.frmSecretaryApproval.SignTo === '' || $scope.frmSecretaryApproval.SignTo === undefined || $scope.frmSecretaryApproval.SignTo === 'undefined') {
            toastr.warning("প্রাপোক নির্বাচন করুন");
            return false;
        }
        $scope.SaveDb = {};
        ...
        $scope.SaveDb.ParlSessID = $scope.frmSecretaryApproval.ParliamentSession;

        $scope.SaveDb.SecApproveDetail = $scope.ApproveDetail;
        $scope.SaveDb.SecApproveDate = $scope.ApproveDate;
        $scope.SaveDb.SecApproveStatus = $scope.frmSecretaryApproval.AppStatus;
        $scope.SaveDb.SendTo = $scope.frmSecretaryApproval.SignTo;
        $scope.SaveDb.DataMode = $scope.DataMode;
    ...
    $scope.SaveDataDraft = function () {
        ...
        $http({
            method: "post",
            url: MyApp.rootPath + "ResolutionApproval/DraftSecretary",
            datatype: "json",
            data: JSON.stringify($scope.SaveDb)
        }).then(function (response) {
            if (response.data.Status === "Yes") {
    ...
    $scope.Reset = function () {
        $scope.gridResolutionOptions.data = [];
        $scope.frmSecretaryApproval.ParliamentSession = "";
        $scope.uiID = "";
        $scope.uiCode = "";
