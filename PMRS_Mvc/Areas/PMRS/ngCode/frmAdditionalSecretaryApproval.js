app.controller("myCtrl", function ($scope, $http, $filter) {
    ...
    $scope.GetWaitingListForAdditionalSecretary = function () {
        $scope.gridResolutionOptions.data = [];
        $http({
            method: "POST",
            url: MyApp.rootPath + "ResolutionApproval/GetWaitingListForAdditionalSecretary",
            data: { session: $scope.frmAdditionalSecretaryApproval.ParliamentSession, DataMode: $scope.DataMode }
        }).then(function (response) {
    ...
    $scope.GetSearchForEditAdditionalSecretary = function () {

        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetSearchForEditAdditionalSecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetPostedHistoryListByAdditionalSecretary = function () {

        $scope.gridDepartmentOptions.data = [];
        $http({
            method: "GET",
            url: MyApp.rootPath + "ResolutionApproval/GetPostedHistoryListByAdditionalSecretary"
        }).then(function (response) {
            if (response.data.length > 0) {
    ...
    $scope.GetDraft = function () {
        ...
        $http({
            method: "POST",
            url: MyApp.rootPath + "ResolutionApproval/GetWaitingListForAdditionalSecretary",
            data: { session: $scope.frmAdditionalSecretaryApproval.ParliamentSession, DataMode: "Draft" }
        }).then(function (response) {
            if (response.data.length > 0) {
                $('#DepartmentModal').modal('toggle');
                $scope.gridDepartmentOptions.data = response.data;
    ...
    var columnDepartmentList = [
        ...
        { name: 'AcceptanceComment', displayName: "গ্রহনযোগ্যতা", visible: false },
        { name: 'RDNo', displayName: "আর ডি নং", width: 100 },
        { name: 'AddSecApproveStatus', displayName: "Status", width: 120 },
        { name: 'AddSecApproveDetail', displayName: "Status", visible: false },
        { name: 'AddSecApproveDate', displayName: "Status", visible: false },
    ...
    $scope.rowDblClickCompApproval = function (row) {
        ...
        $scope.AcceptanceComment = row.entity.AcceptanceComment;
        $scope.RDNo = row.entity.RDNo;
        $scope.frmAdditionalSecretaryApproval.ParliamentSession = row.entity.ParlSessID;

        $scope.ApproveDetail = row.entity.html;
        $scope.ApproveDate = $filter('FullDateTime')(row.entity.AddSecApproveDate);
        $scope.frmAdditionalSecretaryApproval.AppStatus = row.entity.AprID;
        $scope.GetResolutionLog(row.entity.MemberResolutionID);
    ...
    $scope.DirectSave = function (row) {
        ...
        $scope.SaveDb.AddSecApproveDetail = (row.entity.html == '' || row.entity.html == null || row.entity.html == "null") ? row.entity.MemberResolutionDetail : row.entity.html; 
        $scope.SaveDb.AddSecApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
        $scope.SaveDb.AddSecApproveStatus = "1";
        $scope.SaveDb.SendTo = $scope.frmAdditionalSecretaryApproval.SignTo;
        ...
        if ($scope.uiID === '' || typeof $scope.uiID === 'undefined' && $scope.SaveDb.SendTo != '' && $scope.SaveDb.SendTo != 'undefined' && $scope.SaveDb.SendTo != undefined) {
            $http({
                method: "post",
                url: MyApp.rootPath + "ResolutionApproval/UpdateASApproval",
                datatype: "json",
    ...
    $scope.MultipleSaveData = function () {
        ...
        for (var i = resolutionList.length - 1; i >= 0; i--) {
            ...
            if (resolutionList[i].Selected === true) {
                ...
                $scope.SaveDb.AddSecApproveDetail = (resolutionList[i].html == '' || resolutionList[i].html == null || resolutionList[i].html == "null") ? resolutionList[i].MemberResolutionDetail : resolutionList[i].html; ;
                $scope.SaveDb.AddSecApproveDate = (dt.getFullYear() + '-' + dt.getMonth() + '-' + dt.getDate());
                $scope.SaveDb.AddSecApproveStatus = "1";
                $scope.SaveDb.SendTo = $scope.frmAdditionalSecretaryApproval.SignTo;
                $scope.SaveDb.DataMode = $scope.DataMode;
    ...
    $scope.SaveData = function () {
        ...
        if ($scope.frmAdditionalSecretaryApproval.AppStatus === '' || $scope.frmAdditionalSecretaryApproval.AppStatus === undefined) {
            toastr.warning("সিদ্ধান্ত-প্রস্তাবের মতামত প্রদান করুন");
            return false;
        }
        if ($scope.frmAdditionalSecretaryApproval.SignTo === '' || $scope.frmAdditionalSecretaryApproval.SignTo === undefined) {
            toastr.warning("প্রাপোক নির্বাচন করুন");
            return false;
        }
        $scope.SaveDb = {};
        ...
        $scope.SaveDb.ParlSessID = $scope.frmAdditionalSecretaryApproval.ParliamentSession;

        $scope.SaveDb.AddSecApproveDetail = $scope.ApproveDetail;
        $scope.SaveDb.AddSecApproveDate = $scope.ApproveDate;
        $scope.SaveDb.AddSecApproveStatus = $scope.frmAdditionalSecretaryApproval.AppStatus;
        $scope.SaveDb.SendTo = $scope.frmAdditionalSecretaryApproval.SignTo;
        $scope.SaveDb.DataMode = $scope.DataMode;
        $http({
            method: "post",
    ...
    $scope.SaveDataDraft = function () {
        ...
        $http({
            method: "post",
            url: MyApp.rootPath + "ResolutionApproval/DraftAddSecretary",
            datatype: "json",
            data: JSON.stringify($scope.SaveDb)
        }).then(function (response) {
            if (response.data.Status === "Yes") {
    ...
    $scope.Reset = function () {
        ...
        $scope.RDNo = "";
        $scope.frmAdditionalSecretaryApproval.ParliamentSession = "";
